...
interface PasswordRule { id: string; label: string; test: (pw: string) => boolean; }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ marginTop: 10 }}>
      <div style={{ display: 'flex', gap: 4, marginBottom: 5 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ flex: 1, height: 3, borderRadius: 99, background: 'rgba(255,255,255,0.07)', overflow: 'hidden' }}>
            <motion.div animate={{ scaleX: strength >= i ? 1 : 0, backgroundColor: color }} transition={{ duration: 0.3 }}
      ...
      {password.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <motion.span key={label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ fontSize: 11, fontWeight: 600, color, letterSpacing: 0.5 }}>{label}</motion.span>
        </div>
      )}
    </motion.div>
  );
...
const RuleChecklist = ({ password }: { password: string }) => (
  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
    style={{ marginTop: 10, padding: '12px 14px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px 12px' }}>
      {PASSWORD_RULES.map(rule => {
        const ok = rule.test(password);
        return (
          <motion.div key={rule.id} animate={{ opacity: ok ? 1 : 0.5 }} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <motion.div animate={{ backgroundColor: ok ? '#00f5a0' : 'rgba(255,255,255,0.12)', scale: ok ? [1, 1.3, 1] : 1 }} transition={{ duration: 0.25 }}
              style={{ width: 15, height: 15, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <AnimatePresence>
                {ok && (
                  <motion.svg key="check" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} width="8" height="8" viewBox="0 0 8 8" fill="none">
                    <path d="M1.5 4L3 5.5L6.5 2" stroke="#000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </motion.svg>
                )}
              </AnimatePresence>
            </motion.div>
            <span style={{ fontSize: 11, color: ok ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.28)', lineHeight: 1.4 }}>{rule.label}</span>
          </motion.div>
        );
      })}
    </div>
...
const SignUp = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (failedRules.length > 0) { setShowRules(true); error('Weak password', failedRules[0].label); return; }
    setLoading(true);
    try {
  ...
  const inputStyle = (field: string): React.CSSProperties => ({
    ...
    borderRadius: 13, padding: field === 'password' ? '13px 44px 13px 16px' : '13px 16px',
    color: '#fff', fontSize: 14, fontFamily: "'DM Sans',sans-serif",
    outline: 'none', transition: 'all 0.2s', caretColor: '#00f5a0',
  });
            ...
            >
              ...
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.18 }} style={{ marginBottom: 14 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <motion.div animate={{ opacity: focusedField === 'username' ? 1 : 0 }} transition={{ duration: 0.2 }}
                  ...
                  <input type="text" name="username" placeholder="yourname" onChange={handleChange}
                    onFocus={() => setFocusedField('username')} onBlur={() => setFocusedField(null)} onKeyDown={handleKeyDown}
              ...
              <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.30 }} style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <button type="button" onClick={() => setShowRules(p => !p)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: showRules ? '#00f5a0' : 'rgba(255,255,255,0.25)', fontSize: 11, fontFamily: "'DM Sans',sans-serif", padding: 0, transition: 'color 0.2s' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <motion.div animate={{ opacity: focusedField === 'password' ? 1 : 0 }} transition={{ duration: 0.2 }}
                    style={{ position: 'absolute', inset: -1, borderRadius: 14, background: 'linear-gradient(135deg,rgba(0,245,160,0.3),rgba(0,217,245,0.3))', zIndex: 0 }} />
                  <motion.div animate={{ opacity: allRulesPass && touched ? 1 : 0 }} transition={{ duration: 0.35 }}
                    style={{ position: 'absolute', inset: -1, borderRadius: 14, background: 'rgba(0,245,160,0.25)', boxShadow: '0 0 16px rgba(0,245,160,0.2)', zIndex: 0 }} />
                  <input type={showPassword ? 'text' : 'password'} name="password" placeholder="••••••••" onChange={handleChange}
                    onFocus={() => { setFocusedField('password'); setShowRules(true); }} onBlur={() => setFocusedField(null)} onKeyDown={handleKeyDown}
                ...
                <AnimatePresence>{touched && <StrengthMeter password={formData.password} />}</AnimatePresence>
                <AnimatePresence>{showRules && touched && <RuleChecklist password={formData.password} />}</AnimatePresence>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.38 }}>
                <motion.button onClick={handleSubmit} disabled={loading}
                  ...
                  style={{ width: '100%', padding: '15px', borderRadius: 13, border: 'none', background: loading ? 'rgba(0,217,245,0.3)' : 'linear-gradient(135deg,#00d9f5,#00f5a0)', color: '#000', fontSize: 15, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", cursor: loading ? 'not-allowed' : 'pointer', position: 'relative', overflow: 'hidden' }}>
                  {!loading && (
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.52 }}
                ...
                <Link to="/auth/login" style={{ color: '#00f5a0', textDecoration: 'none', fontWeight: 600 }}
